...
%% Title set 2026-08-16 per user confirmation, matching branches (b)/(c) of
%% the title decision rule (neither MELD nor DAiSEE fired branch (a) --
%% see \S\ref{sec:pre-registration}/\S\ref{sec:results}). Subject to a final
%% alternative the user may still confirm.
\title{Does Identity Persistence Matter? Pre-Registered Null Results for
  Entity-Centric Video Tokenization}
%% Repo push verified 2026-08-16: github.com/multi-modal-rtm/ept is public
%% (unauthenticated HTTP 200, isPrivate=false via `gh repo view`) and
%% origin/main's HEAD (d25fbc0) matches this working tree's HEAD exactly --
%% the earlier 403 from this session is resolved. Still confirm anonymous-
%% review requirements don't prohibit an identifying URL before this note
%% ships in a submitted (as opposed to camera-ready) copy.
...
\keywords{video tokenization, dataset leakage, pre-registration, null
  results, entity-centric representation}
...
\input{sections/dataset_audit}

\section{Method}
\input{sections/method}

\section{Experimental Setup}
\input{sections/pre_registration}
...
OUC-CGE~\cite{lu2025ouccge}, MELD~\cite{poria2019meld}, and
DAiSEE~\cite{gupta2016daisee} are used under their respective stated
release terms: OUC-CGE under CC-BY-4.0; DAiSEE under its data-use agreement
(research use only, no redistribution to third parties, and citation of the
original DAiSEE papers), agreed to by the researchers who obtained it for
this project. The dataset audit in \S\ref{sec:dataset-audit} concerns
split-protocol properties --- near-duplicate content assigned across
train/val/test partitions --- not individuals, and no attempt was made to
identify, track, or re-identify any person appearing in any corpus beyond
the anonymized entity-tracking already required by this paper's own method.
The OUC-CGE leakage finding reported in \S\ref{sec:dataset-audit} is a
property of the released partition, not of the underlying recordings,
their collection, or their authors.
