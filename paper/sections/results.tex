...
%% outputs/meld_phase6_gpu_detection_attempt.json: root_cause
    ...
    12.8 runtime libraries (matching the CUDA-12.8-pinned
    PyTorch~\cite{paszke2019pytorch} build used throughout, itself pinned
    for Blackwell/sm\_120 support); the two are
...
$621.5$ GFLOPs/clip against A1's own
%% results/efficiency_budget_sweep.csv: e=6,s=4 row, total_model_gflops_per_clip=1261.45 -> 1261.5
%% (total model: encoder + fusion attention + head; the encoder-only figure quoted
%% above this subsection, $1261.0$, excludes the ~0.5 GFLOPs of fusion attention and
%% head -- both numbers are correct, they are not the same quantity)
total $1261.5$ (encoder-only: $1261.0$, above). Every one of the 10
budget-sweep cells' CIs exclude the trivial-feature probe; $E{=}1,S{=}2$ is
the smallest by token count that does
