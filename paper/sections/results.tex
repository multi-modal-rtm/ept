...
A3 (temporal-only, $0.4032{\pm}0.0431$) and A4 (social-only,
$0.4111{\pm}0.0311$) both sit close to A1 ($0.4040{\pm}0.0077$,
...
\begin{figure}[t]
  ...
  \caption{Accuracy vs.\ latency by hardware regime (MELD). No panel mixes
    GPU- and CPU-measured numbers; Regime A (GPU-only) is unavailable, not
    estimated (\S\ref{sec:efficiency}).}
  \label{fig:pareto}
\end{figure}
...
. The frozen DINOv2 encoder --- itself attention-based, applied once per
