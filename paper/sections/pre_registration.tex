...
\texttt{docs/DECISION\_RULES.md} (MELD) and
\texttt{docs/DECISION\_RULES\_DAISEE.md} (DAiSEE) before any test-split
evaluation, with amendments appended and dated, never edited in place. This
is stated here as a credibility mechanism, not a formality: every number in
\S\ref{sec:results} that bears on a hypothesis was computed after its
decision criterion was frozen, not the reverse.

\textbf{H1 (persistence, MELD only).} $A1 - A2 \geq \text{EFFECT\_FLOOR}$,
macro-F1. Not defined for DAiSEE ($E{=}1$, \S\ref{sec:method}).

\textbf{H2 (entity structure).} $A1 - A0 \geq \text{EFFECT\_FLOOR}$,
macro-F1 (MELD); macro-F1 restricted to classes 1--3 (DAiSEE, see below).

\textbf{Decision rule (MELD, three branches).} (a) H1 supported: persistence
is the mechanism. (b) $|A1{-}A2| < \text{EFFECT\_FLOOR}$ and H2 supported:
entity localization and token budget matter, persistence does not. (c) $A1 <
A2 - \text{EFFECT\_FLOOR}$: hypothesis refuted. A branch fires only if both
...
\textbf{Decision rule (DAiSEE, H2 only, two conditions).} (a) H2 supported.
(b) $|\text{margin}| < \text{EFFECT\_FLOOR}$: null/underpowered. (c) margin
$< -\text{EFFECT\_FLOOR}$: reversed. Same paired requirement.
...
recorded --- then $\text{EFFECT\_FLOOR} = \max(0.02, 2\times
\text{pooled\_SE})$, rounded up. MELD:
%% docs/DECISION_RULES.md 2026-08-15 amendment ("Effect floor computed"): pooled_SE=0.0170
pooled SE $0.0170 \Rightarrow \text{EFFECT\_FLOOR}{=}0.04$
%% commit a6d19a8 (procedure fixed) / 8c156a9 (floor computed)
(\texttt{a6d19a8}/\texttt{8c156a9}). DAiSEE:
%% docs/DECISION_RULES_DAISEE.md 2026-08-15 amendment: pooled_SE=0.0138
pooled SE $0.0138 \Rightarrow \text{EFFECT\_FLOOR}{=}0.03$
