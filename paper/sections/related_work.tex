...
levels~\cite{gupta2016daisee}; EngageNet's~\cite{singh2023engagenet}
official split is likewise
...
similarity and cluster/connected-component analysis is addressed directly by
prior work, Ramos~et~al.~\cite{ramos2025leakage} (August 2025, roughly a
year before this project's own audit), using a similar toolkit (embedding
...
novel phenomenon; the contribution is the OUC-CGE instance plus a
reusable, dataset-agnostic admission procedure packaged as one script, not the general observation
that clip-level splitting can leak.

\paragraph{Edge video analytics.}
Deploying video models under CPU or edge power and latency budgets typically
