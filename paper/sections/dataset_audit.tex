...
Figure~\ref{fig:ouccge-leakage} shows two representative pairs directly.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig2_ouccge_leakage.pdf}
  \caption{Frames from OUC-CGE~\cite{lu2025ouccge}, CC-BY-4.0. Written
    informed consent for publication of these images was obtained by the
    dataset authors (ethics approval OUC-HM-20231205). Two val/train pairs,
%% outputs/dataset_admission/ouccge/pairs/: pair_00 and pair_02, both cosine similarity 1.000000
    each at masked-mean DINOv2~\cite{oquab2023dinov2} cosine similarity $1.000000$ against its
    cross-split nearest neighbor (Table~\ref{tab:ouccge-dup}): the same
    physical moment of one continuous recording, independently assigned to
    different splits.}
  \label{fig:ouccge-leakage}
\end{figure}
...
(Figure~\ref{fig:ouccge-leakage}; cosine similarity $\geq 0.9999996$,
identical labels, same room, table, students, objects, and pose) shows
these are adjacent temporal segments of one
